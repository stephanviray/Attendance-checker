...
const AttendanceChecker = () => {
  ...
  const handleScan = async ({ type, data }) => {
    // Check if the data is a JSON string (QR code)
    try {
      const parsedData = JSON.parse(data);
      if (parsedData.type === 'employee_qr' && parsedData.employee_id) {
        console.log('Found employee_qr with ID:', parsedData.employee_id);
        
        // Use the employee ID from the QR code
        const employeeId = parsedData.employee_id;
        ...
        if (!profileError && profileData) {
          // Employee found in Supabase but not in local cache
          const employee = {
          ...
          console.log('Added employee to local cache from Supabase:', employee);
      ...
      // Not JSON, continue with the original ID format
      console.log('Not a JSON QR code, using as-is');
    }
    ...
    try {
      ...
      if (!profileError && profileData) {
        ...
        // Update local cache
        const updatedEmployees = {...employees, [data]: employee};
        setEmployees(updatedEmployees);
        saveEmployees(updatedEmployees);
        console.log('Added employee to local cache from Supabase:', employee);
      ...
      console.error('Error checking employee in Supabase:', error);
    }

...
async function fetchEmployeesList() {
    ...
    try {
        ...
        try {
            ...
            // First try with conditions that match THIS USER's employees
            ...
            if (!employeeError && employeeData && employeeData.length > 0) {
                // Filter to only include employees, not the current user
                employeeProfiles = employeeData.filter(profile => 
                    profile.role === 'employee' || 
                    profile.company_id === currentUser.id
                );
                
                console.log('Successfully fetched employees:', employeeProfiles.length);
            } else {
                console.log('First query failed or returned no results:', employeeError);
                fetchError = employeeError;
                
                // Try with company_id filter specifically
                ...
                if (!companyError && companyData && companyData.length > 0) {
                    ...
                    if (!roleError && roleData && roleData.length > 0) {
                        console.log('Successfully fetched employees via role:', roleData.length);
                        employeeProfiles = roleData;
                    } else {
                        console.log('Role query failed or returned no results:', roleError);
                        fetchError = roleError || companyError || employeeError;
                    }
                }
            }
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form id="registerEmployeeForm">
                                ...
                                <div class="mb-3">
                                    <label for="employeeCustomId" class="form-label">Employee ID</label>
                                    <input type="text" class="form-control" id="employeeCustomId">
                                    <div class="form-text">If left blank, an ID will be automatically generated.</div>
                                </div>
                                <div class="mb-3">
...
async function registerEmployee() {
    try {
        ...
        const customId = document.getElementById('employeeCustomId').value.trim();
        ...
        try {
            ...
            try {
                ...
                if (!directInsertError) {
                    ...
                    // Approach 2: Try using RPC function 
                    ...
                    if (!rpcError) {
                        ...
                        // Approach 3: Try inserting into profiles_view as a last resort
